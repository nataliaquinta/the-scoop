...
//functions: updateComment, deleteComment, upvoteComment, downvoteComment 
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body && requestComment.username && database.articles[requestComment.articleId] && requestComment.articleId && database.users[requestComment.username]) {
  const comment = {
  ...
  database.articles[comment.articleId].commentIds.push(comment.id); //undefined
...
function loadDatabase() {
  try {
    var doc = yaml.safeLoad(fs.readFileSync('./src/config/data.yml', 'utf8'));
    console.log(doc);
  } catch (e) {
    console.log(e);
  }
 ...
 function saveDatabase() {
  const databaseYaml = yaml.stringify(database);
 
  fs.writeFile('./src/config/data.yml', databaseYaml, 'utf8', (err) =>{
    if (err) throw err;
  });
 }
